...
func (v *v7) SearchByHash(h, index string) ([]Document, error) {
...
func (v *v7) SearchByFilePath(p, index string) ([]Document, error) {
	f, err := os.Open(p)
	if err != nil {
		return nil, err
	}
...
func (v *v7) MultiSearchByFilePath(ps []string, index string) (map[string][]Document, error) {
	var hashs []string
	for _, p := range ps {
		f, err := os.Open(p)
		if err != nil {
...
func (v *v7) MultiSearch(hs []string, index string) (map[string][]Document, error) {
